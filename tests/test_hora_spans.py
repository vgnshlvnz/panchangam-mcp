...
from panchangam import ephemeris, hora_spans
...
def test_thursday_hora_1_is_jupiter():
    assert hora_spans.horas(date(2026, 9, 24), kl_place())[0].name == "Jupiter"


@pytest.mark.parametrize("day,lord", zip(WEEK, WEEKDAY_LORDS))
def test_hora_1_is_weekday_lord(day, lord):
    assert hora_spans.horas(day, kl_place())[0].name == lord


def test_lords_follow_the_planetary_order():
    spans = hora_spans.horas(SUNDAY, kl_place())
...
def test_spans_are_contiguous_hours_from_sunrise():
    place = kl_place()
    spans = hora_spans.horas(SUNDAY, place)
...
def test_hora_25_would_repeat_hora_1_lord_shifted_by_three():
    ...
    for today, tomorrow in zip(WEEK, WEEK[1:] + [WEEK[0]]):
        first = hora_spans.horas(today, kl_place())[0].name
        nxt = hora_spans.horas(tomorrow, kl_place())[0].name
